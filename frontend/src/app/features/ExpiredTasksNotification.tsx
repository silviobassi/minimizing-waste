...
interface ExpiredTasksNotification {
  title: string;
...
export default function ApprovedTasks() {
  const [expiredTasksNotification, setExpiredTasksNotification] = useState<
    ExpiredTasksNotification[]
  >([]);
  ...
  const loadMoreData = () => {
    const data: ExpiredTasksNotification[] = [];
    for (let i: number = 1; i < 20; i++) {
      data.push({
        ...
        taskType: 'Obras',
        deadline: '22/01/2023 à 22/02/2023',
      });
    }

    setExpiredTasksNotification(data);
  };
                      ...
                      <>
                        <Row justify={'start'}>
                          <Col xs={24} xl={7}>
                            ...
                            <p>
                              <strong>Data da Notificação:</strong> {item.notificationDate}
                            </p>
                            <p>
